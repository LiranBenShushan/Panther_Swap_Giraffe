 ...
 * uses these constants for IDs: {@link #EMPTY}, {@link #BLACK_CHECKER},
...
public class Board {
	...
	/** The number of the checkers */
	public static final int N_Checkers = 32;
	...
	/**
	 * Resets the checker board to the original game state with black checkers on
	 * top and white on the bottom. There are both 12 black checkers and 12 white
	 * checkers.
	 */
	public void reset() {
		...
		for (int i = 0; i < 12; i++) {
			set(i, BLACK_CHECKER);
			set(N_Checkers - 1 - i, WHITE_CHECKER);
		}
	}
	 ...
	 * @return a list of points on the board with the specified ID. If none exist,
	...
	/**
	 * Sets the ID of a black tile on the board at the specified location. If the
	 * location is not a black tile, nothing is updated. If the ID is less than 0,
	 * the board at the location will be set to {@link #EMPTY}.
	 * 
	 ...
	 * @param x the x-coordinate on the board (from 0 to 7 inclusive).
	 * @param y the y-coordinate on the board (from 0 to 7 inclusive).
	 * @return the ID at the specified location or {@link #INVALID} if the location
	 *         is not on the board or the location is a white tile.
	 * @see {@link #get(int)}, {@link #set(int, int)}, {@link #set(int, int, int)}
	 */
	...
	/**
	 * Converts a black tile index (0 to 31 inclusive) to an (x, y) point, such that
	 * index 0 is (1, 0), index 1 is (3, 0), ... index 31 is (7, 7).
	 * 
	 * @param index the index of the black tile to convert to a point.
	 * @return the (x, y) point corresponding to the black tile index or the point
	 *         (-1, -1) if the index is not between 0 - 31 (inclusive).
	 * @see {@link #toIndex(int, int)}, {@link #toIndex(Point)}
	 */
	public static Point toPoint(int index) {
		int y = index / (Board_Size / 2);
		int x = 2 * (index % (Board_Size / 2)) + (y + 1) % 2;
	...
	/**
	 * Sets or clears the specified bit in the target value and returns the updated
	 * value.
	 * 
	 ...
	 * @return the middle point between two points or (-1, -1) if the points are not
	 *         on the board, are not distance 2 from each other in x and y, or are
	 *         on a white tile.
	...
	public static Point middle(int x1, int y1, int x2, int y2) {
		...
		if (x1 < 0 || y1 < 0 || x2 < 0 || y2 < 0 || // Not in the board
				x1 > Board_Size - 1 || y1 > Board_Size - 1 || x2 > Board_Size - 1 || y2 > Board_Size - 1) {
			return new Point(-1, -1);
	...
	public static boolean isValidIndex(int testIndex) {
		return testIndex >= 0 && testIndex < N_Checkers;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < N_Checkers - 1; i++) {
			obj += get(i) + ", ";
		}
		obj += get(N_Checkers - 1);
