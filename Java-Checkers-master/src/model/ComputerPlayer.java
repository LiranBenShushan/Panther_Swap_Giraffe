...
public class ComputerPlayer extends Player {
	...
	/**
	 * The factor used to multiply some weights when the checker being observed is a
	 * king.
	 */
	...
	public void updateGame(Game game, int redTile)
		...
		for (int i = 0; i < n; i++) {
			Move m = moves.get(i);
			if (this.End != -1 && this.End != m.getStartIndex()) {
				continue;
			} else {
				if (bestWeight == m.getWeight()) {
					if (move == 0) {
						this.End = -1;
						game.move(m.getStartIndex(), m.getEndIndex(), redTile);
						return;
					} else {
						move--;
					}
	...
	private List<Move> getMoves(Game game) {
		...
		if (game.getSkipIndex() >= 0) {
			...
			List<Point> skips = MoveGenerator.getSkipsFirst(game.getBoard(), game.getSkipIndex());
			for (Point end : skips) {
		...
		for (Point checker : checkers) {
			...
			List<Point> skips = MoveGenerator.getSkipsFirst(b, index);
			for (Point end : skips) {
		...
		if (moves.isEmpty()) {
			for (Point checker : checkers) {
				int index = Board.toIndex(checker);
				List<Point> movesEnds = MoveGenerator.getMoves(b, index, b.get(index));
	...
	private int getSkipDepth(Game game, int startIndex, boolean isP1Turn)
		...
		List<Point> skips = MoveGenerator.getSkipsFirst(game.getBoard(), startIndex);
		int depth = 0;
		for (Point end : skips) {
			int endIndex = Board.toIndex(end);
//			game.move(startIndex, endIndex, -1);
	...
	/**
	 * Determines the weight of a move based on a number of factors (e.g. how safe
	 * the checker is before/after, whether it can take an opponents checker after,
	 * etc).
	 * 
	 * @param game the current game state.
	...
	private void getMoveWeight(Game game, Move m)
		...
		m.changeWeight(getSafetyWeight(b, changed ? !game.isP1Turn() : game.isP1Turn()));
	}

	/**
	 * Calculates the 'safety' state of the game for the player specified. The
	 * player has 'safe' and 'unsafe' checkers, which respectively, cannot and can
	 * be skipped by the opponent in the next turn.
	 * 
