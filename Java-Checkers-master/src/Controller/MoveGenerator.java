...
public class MoveGenerator {
	 ...
	 * @return the list of points such that the start to a given point represents a
	 *         move available.
	 * @see {@link #getMoves(Board, int)}
	 */
	public static List<Point> getMoves(Board board, Point start, int id) {
		return getMoves(board, Board.toIndex(start), id);
	}
	 ...
	 */
	public static List<Point> getMoves(Board board, int startIndex, int id) {
		// Trivial cases
		...
		addPointsFist(endPoints, p, id, 1);
	...
	public static List<Point> getSkipsFirst(Board board, int startIndex) {
		...
		int id = board.get(startIndex);
		Point p = Board.toPoint(startIndex);
		addPointsFist(endPoints, p, id, 2);
		if(Move.isSkip == true) {
			addPoints(endPoints, p, id, 2);
		}
	...
	public static List<Point> getKingSkips(Board board, int startIndex, Game game) {
		...
		int id = board.get(startIndex);
		Point p = Board.toPoint(startIndex);
		addPoints(DeltaPoints, p, id, 1);
		for (int i = 0; i < DeltaPoints.size(); i++) {
			// Check that the skip is valid
			if (board.get(startIndex) == Board.WHITE_KING) {
				Point end = DeltaPoints.get(i);
				Point endIndex = new Point();
				if (end.x > p.x && end.y > p.y) {
					endIndex.setLocation(end.x + 1, end.y + 1);
				} else if (end.x > p.x && end.y < p.y) {
					endIndex.setLocation(end.x + 1, end.y - 1);
				} else if (end.x < p.x && end.y < p.y) {
					endIndex.setLocation(end.x - 1, end.y - 1);
				} else if (end.x < p.x && end.y > p.y) {
				...
				if (board.get(Board.toIndex(end)) == Board.EMPTY || board.get(Board.toIndex(end)) == Board.WHITE_CHECKER
						|| board.get(Board.toIndex(end)) == Board.WHITE_KING || !Board.isValidPoint(end)
						|| !MoveLogic.isValidMove(game, startIndex, Board.toIndex(endIndex))) {
					DeltaPoints.remove(i--);
				}
			} else if (board.get(startIndex) == Board.BLACK_KING) {
				Point end = DeltaPoints.get(i);
				Point endIndex = new Point();
				if (end.x > p.x && end.y > p.y) {
					endIndex.setLocation(end.x + 1, end.y + 1);
				...
				if (board.get(Board.toIndex(end)) == Board.EMPTY || board.get(Board.toIndex(end)) == Board.BLACK_CHECKER
						|| board.get(Board.toIndex(end)) == Board.BLACK_KING || !Board.isValidPoint(end)
						|| !MoveLogic.isValidMove(game, startIndex, Board.toIndex(endIndex))) {
					DeltaPoints.remove(i--);
				}
			}
	...
	public static void addPointsFist(List<Point> points, Point p, int id, int delta) {
